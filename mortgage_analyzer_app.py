...
st.caption("Analyze mortgage payments, principal, interest, PMI, remaining balance, and extra payment impact.")

loan_amount = st.number_input("Original Loan Amount ($)", min_value=0.0, value=300000.0, step=1000.0)
home_value = st.number_input("Home Value / Purchase Price ($)", min_value=0.0, value=375000.0, step=1000.0)
...
pmi_cutoff_percent = st.number_input("Stop PMI When Loan Balance Reaches This % of Home Value", min_value=0.0, max_value=100.0, value=80.0, step=1.0)

monthly_rate = annual_rate / 100 / 12
total_months = int(loan_term_years * 12)
pmi_cutoff_balance = home_value * (pmi_cutoff_percent / 100)
